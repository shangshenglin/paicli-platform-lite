...
Lite 版是单机单租户，SQLite WAL 提供并发读取和短事务写入。`schema_migrations` 记录幂等版本：5–7 为委派、公平队列和多模态附件，8 为分层 Memory，9 为模型用量治理，10 为审批策略与 P0 管理状态，11 为 P1 模板、模型方案、预算、调度和通知状态，12 为评测套件、用例、Execution、Trial 与基线，13 为生产级 Run 状态机、工具 Effect/恢复、ModelAttempt、预算预留和通知 Outbox。升级时会先终止历史重复活跃 Run，再建立每 Session 单活跃 Run 的唯一约束。
