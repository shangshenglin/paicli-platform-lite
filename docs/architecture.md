...
PaiCLI Server
  ...
  -> PlanExecutionService：Step 调度 / ReAct Run 绑定 / Async Job / Validation Check
...
- `validation_checks` 保存每个 Step 的 Done Criteria、实际结果和证据，供 Console 与最终回答引用。
...
计划启动后，`PlanExecutionService` 会领取 `READY` Step：`REACT` Step 创建普通 Run 并在 Run 终态后回填 Step；`ASYNC`/`ASYNC_JOB` Step 同时登记 Async Job，并通过 Job 关联 Run；`NONE` Step 可直接完成；`MANUAL`/`USER_APPROVAL` Step 进入等待人工状态。Run 完成会推进依赖满足的后续 Step，并把 Done Criteria 对应的 Validation Check 标记为通过或失败。
