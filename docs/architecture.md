...
版本化官方 Starter Pack `2.1.0` 位于 classpath `evaluations/starter-pack.json`，包含 9 个 Suite、41 个 Case。Suite 保存 `dataset_version`，Case 保存 `assertion_spec_json`、`fixture_spec_json` 与 `judge_spec_json`。`RuleEvaluationFixtureService` 为每个 Trial 的唯一 workspace 写入 Case 声明的固定文件并在快照记录规范路径、字节数和 SHA-256，还可创建可检索的临时 Session 历史、workspace-scoped Memory、临时 Knowledge 文档、绑定内部 Plan，并要求 AgentTeam Policy；创建的持久化 Session/Memory/Knowledge/Plan ID 冻结进 Trial 快照，评分后幂等清理，避免评测夹具污染用户状态。README/POM 总结、间接文件注入、失败命令等用例同时核对指定工具参数、真实工具结果与最终回答事实，不能由空工作区或只调用工具通过。高级和对抗安全用例仍默认停用，只有显式满足前置条件后启用。

Starter Pack 安装器只原位升级未人工编辑且仍由旧官方版本托管的 Suite/Case；数据集版本迁移前创建、后来被回填为 `custom-v1` 或已经发生部分升级的“官方·”套件也可恢复。为避免时间戳漂移覆盖用户内容，资源中只保存历代 1.0–2.0 官方 Case 定义的 SHA-256；签名覆盖 Prompt、工具/回答规则、预算、断言、Fixture 和 Judge，任何这些字段变化都会使签名失配并保留原 Case。`enabled` 明确排除在签名外，内容升级后继续使用用户当前启停状态；新版本淘汰且签名匹配的旧 Case 才会停用、不删除。安全、RAG、History、Plan、Memory、AgentTeam 与 Harness 用例不再把标准答案标记写入 Prompt；仅精确格式与稳定性基线保留显式标记。新增“官方·09 对抗安全”以直接越权、RAG 污染、编码文件注入和 Memory 权限污染验证可观测的泄露或越权证据。
