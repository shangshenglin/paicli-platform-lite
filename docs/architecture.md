...
`EvaluationService` 在读取报告时同步终态 Trial，并从 Run、Message、ToolCall 和 ModelUsage 构建结构化评分快照。第一版评分器只处理确定性规则：Run 终态、必需/禁止工具、回答包含/排除项、工具数、Token、耗时，以及人工确认基线中的关键工具与 150% 资源退化阈值。Execution 只有在全部 Trial 达到阈值时通过；运行中报告保持可刷新，不阻塞 Worker 等待。

`evaluation_baselines` 保存人工确认的来源 Run、最终回答、工具名称序列、Token 和耗时。基线不持久化为新的模型上下文，也不把原始 reasoning 作为严格相等条件；这避免模型升级时因合法路径变化产生大量伪回归。后续 Rubric/LLM Judge 可以作为新的评分器类型扩展，但不能替代可确定验证的安全和 Runtime 契约。
...
Lite 版是单机单租户，SQLite WAL 提供并发读取和短事务写入。`schema_migrations` 记录幂等版本：5–7 为委派、公平队列和多模态附件，8 为分层 Memory，9 为模型用量治理，10 为审批策略与 P0 管理状态，11 为 P1 模板、模型方案、预算、调度和通知状态，12 为评测套件、用例、Execution、Trial 与基线。
...
OpenAI 兼容客户端会执行限流，并且只在 SSE 响应被接受前重试，避免重复流式 delta。可重试 HTTP 失败采用指数退避，也可按 Run 的模型方案切换后备模型或在失败后选择另一方案重试。每 Run 在新模型调用前检查步骤和 Token 预算，但已持久化的可恢复 ToolCall 仍会执行。`model_usage` 记录每轮模型、Token、缓存、耗时和重试；Actuator 暴露排队 Run、待审批、待提取 Memory、活动 SSE、模型重试和工具失败指标。Run id 是 Event、Audit、ToolCall 和 Artifact 的统一关联键。
