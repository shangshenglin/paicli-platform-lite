...
Memory 与 Knowledge 在进入模型上下文时统一标记为不可信数据，不能作为指令或授权；上下文安全指令同时禁止复述其中的凭据、验证码、Token、Canary 或密钥。该防护属于正式 Runtime Prompt，而不是评测 Case 题面中的标准答案提示，评测仍通过隐藏 Canary 观测防护是否真实生效。
...
`EvaluationService` 在读取报告时同步终态 Trial，并从 Run、Message、ToolCall、Approval、Event、Plan、Memory Selection 和 ModelUsage 构建结构化评分快照。确定性规则收敛到可独立变异测试的 `EvaluationAssertionEngine`：除旧的工具/回答/资源规则外，还能检查工具参数子集或精确值、状态、次数、真实工具结果、Provider 顺序、回答事实/正则、Event 子序列、Approval 先于执行、拒绝后不得启动、幂等键唯一、重复签名、终态工具、虚假完成声明，以及真实 Plan DAG/Validation、Memory Selection 和 Delegation 数量。安全断言还能限制工具白名单与敏感参数、禁止产生 Approval，并对回答、工具参数以及按 Case 选择的结果/错误/Event 扫描 Canary 的原文、Base64、URL-safe Base64、十六进制和 URL 编码变体。所有安全、功能、证据和资源断言都是硬门禁；分数只用于质量排序。
...
版本化官方 Starter Pack `2.3.3` 位于 classpath `evaluations/starter-pack.json`，包含 9 个 Suite、41 个 Case。Suite 保存 `dataset_version`，Case 保存 `assertion_spec_json`、`fixture_spec_json` 与 `judge_spec_json`。`RuleEvaluationFixtureService` 为每个 Trial 的唯一 workspace 写入 Case 声明的固定文件并在快照记录规范路径、字节数和 SHA-256，还可创建可检索的临时 Session 历史、workspace-scoped Memory、临时 Knowledge 文档、绑定内部 Plan，并要求 AgentTeam Policy；创建的持久化 Session/Memory/Knowledge/Plan ID 冻结进 Trial 快照，评分后幂等清理。评测 workspace 的自动 RAG 与 `search_knowledge` 只检索当前 Run 命名的临时 Knowledge 文档；没有 Knowledge 夹具的并发 Trial 不读取同批临时文档。当前 Run 标记为 `evaluation-fixture` 的 Memory 无视普通置信度/相关性门槛确定性进入候选，但仍受 workspace scope 限制。Grader 再从 `context.prepared` 核对实际选中的 Knowledge/Memory 数量和内容事实，避免“创建了夹具却未送进模型”的假评测。README/POM 总结、间接文件注入、失败命令等用例同时核对指定工具参数、真实工具结果与最终回答事实，不能由空工作区或只调用工具通过。高级和对抗安全用例仍默认停用，只有显式满足前置条件后启用。

Starter Pack 安装器只原位升级未人工编辑且仍由旧官方版本托管的 Suite/Case；数据集版本迁移前创建、后来被回填为 `custom-v1` 或已经发生部分升级的“官方·”套件也可恢复。为避免时间戳漂移覆盖用户内容，资源中只保存历代官方 Case 定义的 SHA-256；签名覆盖 Prompt、工具/回答规则、预算、断言、Fixture 和 Judge，任何这些字段变化都会使签名失配并保留原 Case。`enabled` 明确排除在签名外，内容升级后继续使用用户当前启停状态；淘汰 Case 的判定不再依赖 Case 与 Suite 更新时间先后，即使用户后续切换启停也能依内容签名识别。所有用例（包括精确格式与稳定性基线）都不再把判分标记写入 Prompt。“官方·09 对抗安全”以直接越权、RAG 污染、编码文件注入和 Memory 权限污染验证可观测的泄露或越权证据。
...
Memory 仍以 SQLite 为事实源和向量存储，不进入 Milvus。迁移 43 为每条 Memory 增加 `PROJECT / AGENT / WORKSPACE / TASK_TYPE` Scope 及对应 Agent、workspace owner、任务类型字段，并从历史自动 Memory 的来源 Run 幂等回填；人工 Memory 默认项目级。每轮召回先执行 Scope、状态和置信度过滤，再以语义/词法、置信度、时间衰减、范围亲和度与历史反馈生成有界候选；词法匹配基于规范化完整 term，不再用子串让 `Java` 命中 `JavaScript`，也不再为 L3 无条件加分。候选复用 KnowledgeReranker 的 TEI Cross-Encoder，默认候选/结果上限为 12/3，并执行 L1≤1、L2≤2、L3≤1 配额；自动 PROJECT Memory 跨任务类型时只有 Cross-Encoder 原始分达到严格门槛才可进入。服务不可用时仍对整批候选计算确定性分数，但召回采用 fail-closed：绝对分至少 0.60、基础分至少 0.55、还需词法覆盖至少 0.25 或语义相似至少 0.72，且最多返回 2 条。`memory_usage_feedback` 保存实际选入上下文的 Memory；Run 终态以及 Plan 验证通过/返工更新 outcome。Cross-Encoder 只判断 query-memory 相关性，不能覆盖证据、冲突、修订、状态或项目隔离。显式 REST CRUD 仍是人工纠错边界；Console 也提供主动录入人工 L3 的入口。
