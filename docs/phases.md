...
- Evidence：`RunEvidenceCollector` 通过与 SQLite terminal envelope 共用的 `RunEvidenceDecoder` 统一收集 files/commands/tests/artifacts/workspaceMutations/lastMutationOrdinal；Sandbox command metadata 提供 `changedPaths`，`BuildCommandClassifier` 以生成型/明确直接写入/未知不可信三态和 path attribution 保守判定命令 mutation，`TestCommandClassifier` 高精度测试族分类；`write_file` 结构化 before/after sha256。
- 验证：`RunVerificationService` 按合同模式验证，非空 writeScope 要求全部归属 mutation path 位于 scope，required tests 在最后一次 mutation 后通过、不同 TestFamily 互不覆盖。
...
- [x] AgentResult/SQLite delegation envelope 输出带 `changed_paths` 的 `workspace_mutations`，让命令级工作区变更在 Child CompletionVerifier 与 Parent AgentResultValidator 之间保持同一语义；非空 `writeScope` 强制全部归属路径位于 scope，无路径/截断/生成物路径不能满足 scoped contract。`BuildCommandClassifier` 三态默认拒绝未知命令、不安全复合命令及 `rm/cp/mv/touch` 等泛文件操作；AgentResult 以 `ordinal/after_last_mutation` 同步父子按 family 取最新测试结果的时序。SQLite terminal、阶段 delivery gate 与 DeliveryManifest 统一消费 RunEvidence，`tool_result` 不外泄为交付物，也不按共享 workspace 时间戳归属文件；分类器拒绝换行、后台 `&`、no-run/skip、Gradle exclude/dry-run、依赖安装、CLI help/version、孤立 Maven selector 与 Gradle `testClasses`/`checkstyle*` 误报，AcceptedSnapshot 聚合根与全部阶段交付。
