...
public record MemoryProperties(boolean autoExtract, int retrievalTopK, int retrievalCandidateLimit,
    ...
    public MemoryProperties {
        retrievalTopK = retrievalTopK <= 0 ? 3 : Math.min(retrievalTopK, 20);
        retrievalCandidateLimit = retrievalCandidateLimit <= 0 ? 12
                : Math.max(retrievalTopK, Math.min(retrievalCandidateLimit, 100));
        minRelevance = minRelevance <= 0 || minRelevance >= 1 ? 0.50 : minRelevance;
