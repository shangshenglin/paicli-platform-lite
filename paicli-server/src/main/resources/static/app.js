...
// ================= PRD Analysis =================
const prdState = { taskId: null, attachment: null, contract: null, supporting: [] };

async function openPrdAnalysis() {
  $('prdProject').textContent = '当前项目：' + currentProjectKey();
  $('prdDialog').showModal();
  await refreshPrdTasks();
}

async function refreshPrdTasks() {
  const list = $('prdTaskList');
  list.textContent = '加载中…';
  try {
    const tasks = await api('/v1/prd-analysis/tasks?projectKey=' + encodeURIComponent(currentProjectKey()) + '&limit=50');
    renderPrdTaskList(tasks || []);
    if (prdState.taskId) openPrdDetail(prdState.taskId);
  } catch (error) {
    list.textContent = '';
    list.append(element('div', 'managed-item', '加载失败：' + error.message));
  }
}
...
function renderPrdTaskList(tasks) {
  ...
  list.textContent = '';
  if (!tasks.length) {
    list.append(element('div', 'managed-item', '暂无 PRD 分析任务，点击“新建 PRD 分析”开始。'));
    return;
  }
  for (const task of tasks) {
    const item = workbenchItem(task.title,
      '项目 ' + task.projectKey + ' · ' + task.status + ' · 阶段 ' + task.stage
      + ' · 节点 ' + task.nodesCompleted + '/' + task.nodesTotal
...
async function startPrd(taskId) {
  try {
    await api('/v1/prd-analysis/tasks/' + encodeURIComponent(taskId) + '/start', { method: 'POST' });
    await refreshPrdTasks();
  } catch (error) { showNotice(error.message, true); }
}

async function cancelPrd(taskId) {
  if (!confirm('确认取消该 PRD 分析任务？')) return;
  try {
    await api('/v1/prd-analysis/tasks/' + encodeURIComponent(taskId) + '/cancel', { method: 'POST' });
    await refreshPrdTasks();
  } catch (error) { showNotice(error.message, true); }
}
...
async function createPrdPlan(taskId) {
  try {
    const plan = await api('/v1/prd-analysis/tasks/' + encodeURIComponent(taskId) + '/plans', {
      method: 'POST', body: JSON.stringify({})
    });
    showNotice('已创建实施计划：' + plan.id + '（可在“效率工作台”或计划视图查看）');
  } catch (error) { showNotice(error.message, true); }
}
...
async function uploadPrdFile(file) {
  ...
  form.append('file', file);
  try {
    const attachment = await api('/v1/sessions/' + state.sessionId + '/attachments/documents', { method: 'POST', body: form });
    return attachment;
  } catch (error) {
...
async function openPrdCreate() {
  prdState.attachment = null; prdState.contract = null; prdState.supporting = [];
  $('prdTitle').value = '';
  ...
  try {
    const task = await api('/v1/prd-analysis/tasks', {
      ...
      body: JSON.stringify({
        ...
        projectKey: currentProjectKey(),
        title: title,
        prdAttachmentId: prdState.attachment.id,
        sourceContractAttachmentId: prdState.contract ? prdState.contract.id : null,
        supportingAttachmentIds: prdState.supporting.map(item => item.id),
        maxParallelism: parallelism
      })
    });
    $('prdCreateDialog').close();
...
function setPrdFormStatus(id, message) {
  ...
  node.className = 'capability-status' + (message ? ' error' : '');
}

function prdTabButton(label, active) {
  const button = element('button', 'secondary prd-tab' + (active ? ' active' : ''), label);
  return button;
}

function renderPrdDetail(detail) {
  const box = $('prdDetail');
  box.textContent = '';
  const task = detail.task;
  const header = element('div', 'managed-item');
  const main = element('div', 'managed-main');
  const titleLine = element('strong', '', task.title);
  main.append(titleLine, element('small', 'snippet',
    '状态 ' + task.status + ' · 阶段 ' + task.stage
    + (task.lastError ? ' · ' + task.lastError : '')));
  header.append(main, element('div', 'managed-actions'));
  if (task.status === 'DRAFT') {
    const b = element('button', 'primary', '开始');
    b.onclick = () => startPrd(task.id);
    header.querySelector('.managed-actions').append(b);
  }
  if (['INGESTING', 'MAPPING', 'ANALYZING', 'RECONCILING', 'VERIFYING', 'PACKAGING', 'WAITING_USER'].includes(task.status)) {
    const b = element('button', 'secondary', '取消');
    b.onclick = () => cancelPrd(task.id);
    header.querySelector('.managed-actions').append(b);
  }
  if (task.status === 'FAILED') {
    const b = element('button', 'primary', '重试');
    b.onclick = () => retryPrd(task.id);
    header.querySelector('.managed-actions').append(b);
  }
  if (task.status === 'COMPLETED') {
    const b = element('button', 'primary', '生成实施 Plan');
    b.onclick = () => createPrdPlan(task.id);
    header.querySelector('.managed-actions').append(b);
  }
  const refresh = element('button', 'secondary', '刷新');
  ...
  if (task.status === 'WAITING_USER') {
    box.append(renderPrdAnswers(task, detail.questions));
  }

  const tabs = element('div', 'prd-tabs');
  ...
  const sections = {
    ...
    '产物': () => renderPrdArtifacts(detail.artifacts || []),
    'Runs': () => renderPrdRuns(detail.runs || [])
  };
  const keys = Object.keys(sections);
  keys.forEach((key, index) => {
    const tab = prdTabButton(key, index === 0);
    tab.onclick = () => {
      tabs.querySelectorAll('.prd-tab').forEach(node => node.classList.remove('active'));
      tab.classList.add('active');
      content.textContent = '';
      content.append(sections[key]());
    };
    tabs.append(tab);
  });
  box.append(tabs, content);
  content.append(sections['概览']());
}

function renderPrdOverview(detail) {
  const task = detail.task;
  const wrap = element('div', 'managed-list');
  const sources = (detail.sources || []).map(source =>
    '来源 ' + source.sourceType + ' · ' + source.fileName + ' · ' + source.extractionStatus + ' · ' + source.chunkCount + ' chunks');
  const nodeStats = Object.keys(detail.nodeStats || {}).map(key => key + ':' + detail.nodeStats[key]).join(' / ');
  const lines = [
    '项目：' + task.projectKey,
    '最大并发：' + task.maxParallelism + ' · 归并轮次：' + task.reconcileIteration,
    '节点统计：' + (nodeStats || '无'),
    '阻塞问题：' + task.blockingQuestions
  ];
  lines.forEach(line => wrap.append(element('div', 'managed-item', line)));
  sources.forEach(line => wrap.append(element('div', 'managed-item', line)));
  if (task.glossary && task.glossary !== '[]') {
    wrap.append(element('div', 'managed-item', 'Glossary：' + task.glossary));
  }
  return wrap;
}
...
function renderPrdNodes(nodes, stats, taskId) {
  ...
  nodes.forEach(node => {
    const item = workbenchItem(node.title || node.clientKey,
...
async function retryPrdNode(nodeId, taskId) {
  try {
    await api('/v1/prd-analysis/tasks/' + encodeURIComponent(taskId) + '/nodes/' + encodeURIComponent(nodeId) + '/retry', { method: 'POST' });
    await openPrdDetail(taskId);
  } catch (error) { showNotice(error.message, true); }
}

function renderPrdFindings(findings) {
  const wrap = element('div', 'managed-list');
  findings.forEach(finding => {
    const item = workbenchItem(finding.name,
      finding.type + ' · ' + (finding.severity || '-') + ' · ' + finding.status
      + (finding.summary ? ' · ' + finding.summary : ''));
    if (finding.evidence && finding.evidence.length) {
      item.querySelector('.managed-main').append(element('small', 'snippet', '证据 ' + finding.evidence.length + ' 条'));
    }
    wrap.append(item);
  });
  return wrap;
...
function renderPrdQuestions(questions) {
  const wrap = element('div', 'managed-list');
  questions.forEach(question => {
    const item = workbenchItem(question.question,
      question.severity + ' · ' + question.status + (question.answer ? ' · 回答：' + question.answer : ''));
    wrap.append(item);
  });
  return wrap;
}

function renderPrdChecks(checks) {
  const wrap = element('div', 'managed-list');
  checks.forEach(check => {
    const item = workbenchItem(check.checkType + ' [' + check.status + ']',
      check.severity + (check.message ? ' · ' + check.message : ''));
    wrap.append(item);
  });
  return wrap;
}

function renderPrdArtifacts(artifacts) {
  const wrap = element('div', 'managed-list');
  artifacts.forEach(artifact => {
    const item = workbenchItem(artifact.name, artifact.type + ' · ' + artifact.size + ' bytes');
    actionButton(item, '下载', () => { window.open('/v1/artifacts/' + artifact.id + '/download'); });
    wrap.append(item);
  });
  return wrap;
}

function renderPrdRuns(runs) {
  const wrap = element('div', 'managed-list');
  runs.forEach(run => {
...
function renderPrdAnswers(task, questions) {
  const wrap = element('div', 'managed-list');
  const header = element('div', 'managed-item', '需要确认 ' + questions.filter(q => q.severity === 'BLOCKING' && q.status === 'OPEN').length + ' 个阻塞问题：');
  wrap.append(header);
  const answers = {};
  questions.filter(q => q.severity === 'BLOCKING' && q.status === 'OPEN').forEach(question => {
    const item = element('div', 'managed-item');
    const main = element('div', 'managed-main');
    main.append(element('strong', '', question.question));
    const input = element('input', 'prd-answer-input');
  ...
  const submit = element('button', 'primary', '提交回答');
  submit.onclick = async () => {
    const items = Object.keys(answers).filter(id => answers[id].value.trim())
      .map(id => ({ questionId: id, answer: answers[id].value.trim() }));
    if (!items.length) { showNotice('请至少回答一个问题', true); return; }
    try {
      await api('/v1/prd-analysis/tasks/' + encodeURIComponent(task.id) + '/answers', {
        method: 'POST', body: JSON.stringify({ answers: items })
      });
      await openPrdDetail(task.id);
    } catch (error) { showNotice(error.message, true); }
  };
  wrap.append(submit);
  return wrap;
...
// wiring
document.getElementById('prdAnalysis').onclick = openPrdAnalysis;
document.getElementById('closePrd').onclick = () => $('prdDialog').close();
document.getElementById('refreshPrd').onclick = refreshPrdTasks;
document.getElementById('createPrdTask').onclick = openPrdCreate;
document.getElementById('closePrdCreate').onclick = () => $('prdCreateDialog').close();
document.getElementById('prdFile').onchange = async event => {
  const file = event.target.files[0];
  if (!file) return;
  $('prdFileStatus').textContent = '上传中：' + file.name + '…';
  const attachment = await uploadPrdFile(file);
  if (attachment) {
    prdState.attachment = attachment;
    $('prdFileStatus').textContent = '已上传：' + attachment.name + '（' + attachment.id + '）';
  } else {
    $('prdFileStatus').textContent = '上传失败，请重试。';
  }
};
document.getElementById('prdContractFile').onchange = async event => {
  const file = event.target.files[0];
  if (!file) return;
  prdState.contract = await uploadPrdFile(file);
};
document.getElementById('prdSupportFiles').onchange = async event => {
  ...
  prdState.supporting = uploaded;
};
document.getElementById('prdCreateSubmit').onclick = submitPrdCreate;
