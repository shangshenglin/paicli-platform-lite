...
| `schema_migrations` | 数据库版本 | 当前版本 1–14 |
...
当前项目回归测试覆盖 RunProcessor、工具失败恢复、重复工具循环保护、Approval Flow、ContextManager、SQLite Store/WAL 并发、模型重试/Fallback、DeepSeek/多模态请求与 SSE 解析、Docker Driver、Local Driver、API Key、管理端点/OpenAPI 安全、Console 安全头、Artifact 原子写入、SQLite 维护、图片/文档附件、长 Markdown/Tika 提取、结构化分块、混合 RAG、MCP、Skill、Memory、长期效率 Store、模板解析回归，以及评测多 Trial、输出 Token 硬门禁、Baseline、审批不旁路、Starter Pack 幂等安装和 Common/Sandbox 边界，现有 76 项测试通过；评测接口还完成了真实 REST 多 Trial 与基线冒烟验证。
