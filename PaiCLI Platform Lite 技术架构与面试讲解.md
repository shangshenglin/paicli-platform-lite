...
| `plans` / `plan_steps` / `plan_edges` | 持久化计划、步骤和 DAG 依赖 | 计划状态、Step 状态、执行模式、验收标准、依赖边 |
| `plan_revisions` / `plan_events` | Replan 历史与计划事件 | 版本原因、原始 JSON、Plan 内 sequence |
| `schema_migrations` | 数据库版本 | 当前版本 1–15 |
...
当前阶段只把计划启动到根 Step `READY`，并提供创建、生成、批准/启动、取消、Replan、Step retry/skip 和事件查询 API。Step 内部如何创建普通 Run、如何接 Async Job、如何做 Read-only 并行 DAG 和 Validation Checks，是后续阶段；这样能保证第一步不绕过已有 ToolCall 持久化、审批和恢复契约。

面试表达可以这样说：

> 我没有让模型输出一段 Markdown 计划就直接执行，而是把 Plan、Step 和依赖边落到 SQLite。模型只负责给出候选 JSON，Server 负责 schema 校验、id 重映射和 DAG 合法性检查。Plan 是任务层对象，ToolCall 仍是动作层对象，所以后续执行时仍然复用现有 ReAct Run、ToolCall、Approval 和 Event 链路。
...
- 当前只把根 Step 推进到 `READY`，后续再把 Step 内部接入普通 ReAct Run、Async Jobs、并行 DAG 和验证闭环。
...
当前项目回归测试覆盖 RunProcessor、工具失败恢复、重复工具循环保护、Approval Flow、ContextManager、SQLite Store/WAL 并发、模型重试/Fallback、DeepSeek/多模态请求与 SSE 解析、Docker Driver、Local Driver、API Key、管理端点/OpenAPI 安全、Console 安全头、Artifact 原子写入、SQLite 维护、图片/文档附件、长 Markdown/Tika 提取、结构化分块、混合 RAG、MCP、Skill、Memory、长期效率 Store、模板解析回归、Plan Runtime 的 JSON/DAG/Replan 校验，以及评测多 Trial、输出 Token 硬门禁、Baseline、审批不旁路、Starter Pack 幂等安装和 Common/Sandbox 边界，现有 79 项测试通过；评测接口还完成了真实 REST 多 Trial 与基线冒烟验证。
