...
- Step 保存任务级标题、描述、类型、执行模式、验收标准、资源读写集、隔离策略、关键路径权重、workspace 引用和状态。
...
当前 Plan 已经接入受控执行闭环：计划启动后，`PlanExecutionService` 会先回收过期且尚未绑定 Run 的 Step 租约，再按关键路径权重、下游数量和 ordinal 领取 `READY` Step。调度器会根据资源读写集阻止同一 Plan 内的写写/读写冲突；冲突 Step 会写入 `RESOURCE_CONFLICT` 并短暂延后，而不是直接失败。`REACT` Step 创建普通 Run；`ASYNC`/`ASYNC_JOB` Step 会同时登记 `async_jobs` 并进入 `WAITING_JOB`；`NONE` Step 可直接完成，`MANUAL`/`USER_APPROVAL` Step 进入等待人工状态。需要隔离的 Step 会创建内部 Session 和受控 workspace 引用；`GIT_WORKTREE` 当前是 Lite 版目录隔离和后续真实 worktree 工具层预留，不自动执行 merge。

Run 终态只代表执行链路结束，不代表任务验收完成。Step 会先进入 `VALIDATING`，由 `PlanValidator` 按 done criteria 写入 `validation_checks`。验证通过才完成 Step、Async Job 和 Plan，并写入 `agent_feedback` 与过程型 Memory；验证失败会进入 `VALIDATION_FAILED`，保留 failure class 和证据质量，供后续评分、调度策略和人工复盘使用。

详细设计见 [Plan Runtime、Multi-Agent 与 Agent Harness 深度说明](docs/plan-multi-agent-harness.md)。
...
### 8.7 Multi-Agent Harness：为什么不是第二套 Loop

PaiCLI 的 Multi-Agent 设计没有复制一套新的执行器。`spawn_agent` 本身是普通 ToolCall，必须先落库、必要时审批，然后由 `DelegationToolProvider` 创建内部子 Session 和普通子 Run。子 Agent 的模型调用、工具执行、审批、Artifact、预算、恢复和 Event 仍由正式 Runtime 负责。

这条链路可以概括为：

```text
Leader Run
  -> 持久化 spawn_agent ToolCall
  -> Approval
  -> createOrGetDelegation(parent_tool_call_id)
  -> internal child Session / child Run
  -> child Run 普通 ReAct 执行
  -> get_agent_result 汇总 child Run / Artifact / Usage
  -> run_delegations.result_json
```

阶段 2/3/4 已经把 Agent 委派从“开一个子任务”升级为结构化执行信封：`spawn_agent` 可绑定 `plan_id`/`plan_step_id`，并持久化 scope、允许文件、允许工具、输入 artifact、期望输出契约、验收标准、预算、deadline、依赖和禁止操作。`get_agent_result` 不只返回文本摘要，还会把子 Run 状态、Artifact、Token 用量、失败分类和证据写入 `run_delegations.result_json`，供 Leader 汇总、Plan 验证和审计复盘。

为了保证恢复安全，`run_delegations.parent_tool_call_id` 唯一。同一个已审批 `spawn_agent` ToolCall 在恢复时会复用原子 Run，而不是重复创建子任务。内部子 Session 不出现在普通会话列表，父 Run 取消会级联取消后代；深度、子数量、非 Leader 继续派生能力都受协作策略限制。

面试时可以这样讲：

> Multi-Agent 不是让多个模型随便互相聊天，而是把“派生子 Agent”也纳入 ToolCall、Approval 和 SQLite 恢复契约。子 Agent 是普通 Run，所以不会绕过工具审批和沙箱边界；Leader 只通过结构化结果聚合事实。
