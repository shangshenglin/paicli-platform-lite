...
| `task_templates` / `model_profiles` / `usage_budgets` | P1 模板、模型方案和预算治理 | 项目作用域、唯一名称、密钥只记录环境变量名 |
...
| `schema_migrations` | 数据库版本 | 当前版本 1–12 |
...
11. P1 任务模板、模型方案、预算、定时任务、通知与队列治理。
...
| P1 效率能力 | `/v1/productivity/**` 模板、模型方案、用量/预算、队列、定时任务和通知 |
...
- P1 工作台：模板、模型方案、预算/用量、运行队列、定时任务、通知、Session 迁移、Skill 生命周期和 MCP 管理。
- Agent 评测中心：结构化维护套件和用例、启动多 Trial 执行、处理待审批项、查看逐项扣分并晋升基线。
...
4. 报告读取真实 ToolCall、最终回答、模型 Token 和 Run 耗时。危险工具仍停在 `WAITING_APPROVAL`，用户只能批准原 ToolCall 或拒绝，评测不会绕过安全边界。
...
第一版选择确定性评分，而不是直接使用 LLM-as-Judge：初始 100 分；Run 未完成扣 100；缺少必需工具扣 20、出现禁止工具扣 50；缺少回答关键片段扣 15、出现禁止片段扣 50；工具数、Token 或耗时超限各扣 10。人工确认的 Baseline 还会检查关键工具是否缺失，以及 Token/耗时是否超过基线 50%。每次扣分都保存 rule、passed、deduction 和 evidence，结果可审计、可复现。
...
- 项目级 Suite/Case CRUD，支持工具、回答、调用次数、Token 和耗时约束。
- 每个 Case 创建真实内部 Run，并支持 1–10 次 Trial；全部 Trial 通过才让 Execution 通过。
- 确定性逐项扣分、证据报告和人工确认 Baseline，可发现工具行为、Token 与耗时退化。
...
1. 在业务工作台新建一个“危险工具审批”套件，设置 2–3 个 Trial 和通过阈值。
...
5. 将一个已完成 Trial 晋升为 Baseline，再解释后续如何发现工具序列、Token 和耗时退化。
...
当前项目回归测试覆盖 RunProcessor、工具失败恢复、Approval Flow、ContextManager、SQLite Store、模型重试/Fallback、DeepSeek/多模态请求与 SSE 解析、Docker Driver、Local Driver、API Key、管理端点/OpenAPI 安全、Console 安全头、Artifact 原子写入、SQLite 维护、图片/文档附件、长 Markdown/Tika 提取、结构化分块、混合 RAG、MCP、Skill、Memory、P1 Store、模板解析回归，以及评测多 Trial/评分/Baseline/审批不旁路、Starter Pack 幂等安装和 Common/Sandbox 边界，现有 65 项测试通过；评测接口还完成了真实 REST 多 Trial 与基线冒烟验证。
