...
- **智能体专家**：保存专家指令、模型方案、工具/Skill 白名单和输出契约；聊天输入区可选择专家运行，首页提供“普通对话 / 专家协作”模式切换。专家协作启动前可选择复杂度、风险、最多专家数和本次可用专家，Leader Run 会持久化协作策略并在当前 Session 内展示子专家任务看板；普通对话不会自动触发协作。
