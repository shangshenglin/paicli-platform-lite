...
- 可选 API Key 认证、Springdoc OpenAPI 和聊天式 Web Console。
- 停机备份/恢复脚本。
...
```powershell
.\scripts\backup.ps1
.\scripts\restore.ps1 -Archive .\backups\paicli-backup-YYYYMMDD-HHMMSS.zip -Force
```

恢复脚本不会直接删除旧数据，而是先改名保存为 `data.before-restore.*`。
