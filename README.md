...
当前已完成阶段 1–12，包含 64 项自动化测试，并完成真实 Docker 与 Agent 评测 REST 冒烟验证。
...
| 质量闭环 | Agent 评测套件、真实内部 Run、多 Trial、确定性评分、审批不旁路、人工 Baseline |
...
同时完善模型治理：安全重试、指数退避、请求限流、同端点后备模型、步骤/Token 预算、每轮 ModelUsage 和 Micrometer 指标。

### 阶段 9：运维、安全与交付

- 非开发部署可设置 `PAICLI_SECURITY_REQUIRE_API_KEY=true`；缺少 `PAICLI_API_KEY` 时拒绝启动。
...
- SQLite 每小时执行被动 WAL checkpoint；Event/Audit 保留默认关闭，可通过环境变量显式开启。
...
SQLite `schema_migrations` 当前记录版本 1–12：基础 Runtime、reasoning/message archive、思考控制、Session 分组与安全删除、Multi-Agent、公平队列、附件、自动 Memory、ModelUsage、业务工作台、长期效率和 Agent 评测。
...
| 配置族 | 用途 |
|---|---|
| `PAICLI_API_KEY`、`PAICLI_SECURITY_*` | REST、Actuator、OpenAPI 认证和生产启动门禁 |
| `PAICLI_MODEL_*` | Provider、端点、模型、Key、上下文/输出、思考、重试、限流、Fallback 和 Run 预算 |
...
项目模型方案、预算、模板、队列、定时任务、通知、Skill 和 MCP 也可在 Console“业务工作台/能力管理”中维护。
...
- Agent 评测多 Trial、确定性扣分、Baseline、内部 Session 隐藏和审批不旁路。
